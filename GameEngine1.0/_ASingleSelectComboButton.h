...
public:
	...
	_ASingleSelectComboButton(int nPosX, int nPosY) :
		m_pActiveButton(NULL),
	...
	void addComponent(_AComponent *pComponent)
	{
		...
		//		nPosX = this pos x
		pComponent->setPosition(0, getTotal());
		int nNewWidth = Size.X;
		if (Size.X < pComponent->getSize().X)
			nNewWidth = pComponent->getSize().X;
		setSize(nNewWidth, getSize().Y + pComponent->getSize().Y);
		setAlignment(m_nAlignment);	// update alignment to accommodate new width
		//		height = 1
